...
export const isValidCustomerToken = (token: string): boolean => {
  if (token === '') {
...
export const checkMethodIsCorrectPlatformVersion = (
  fnName: string,
  platform: string,
  majorVersion: number,
  minorVersion = 0
 ...
 */
export const androidCreateFragment = (managerName: string, viewId: number) => {
  if (AmaniSDK.sharedInstance.isInitialized) {
    ...
    UIManager.dispatchViewManagerCommand(
      viewId,
      // @ts-ignore
      UIManager[managerName].Commands.create.tostring(),
      [viewId]
    );
  }
};
