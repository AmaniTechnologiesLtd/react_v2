...
class IOSNFC {
  ...
  startWithImageData(imageData) {
    if (!(0, _utils.checkMethodIsCorrectPlatformVersion)('startWithImageData()', 'ios', 13)) return;else return this.platformModule.startIOSNFCCaptureWithBase64Image(imageData);
  }
  ...
  startWithNVIData(nviData) {
    if (!(0, _utils.checkMethodIsCorrectPlatformVersion)('startWithImageData()', 'ios', 13)) return;else return this.platformModule.startIOSNFCCaptureWithNVIModel(nviData);
  }
   ...
   */
  startWithMRZCapture() {
    if (!(0, _utils.checkMethodIsCorrectPlatformVersion)('startWithMRZCapture()', 'ios', 13)) return;else return this.platformModule.startIOSNFCCaptureWithMRZCapture();
  }
